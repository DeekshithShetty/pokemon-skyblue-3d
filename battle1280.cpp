...
float powReduceOfCharmander = 0.0f;		//CHARMANDER s power -0.25 to cope up with decrement
float powReduceOfOddish = 0.0f;		//CHARMANDER s power -0.25 to cope up with decrement
...
void Battle::drawPowerStatusBox(){
	...
	glBegin(GL_QUADS);
	    ...
	    glVertex3f(4.7f, 0.0f,  0.0f);          // Top Right
	    glVertex3f(4.7f, 0.0f, -1.7f);          // Bottom Right
	    glVertex3f(-2.8f, 0.0f, -1.7f);          // Bottom Left
	glEnd();
  	...
  	glBegin(GL_QUADS);                  // Draw A Quad
	    ...
	    glColor3f (0.8, 0.8, 0.8);
	    glVertex3f(4.5f, 0.0f,  0.2f);          // Top Right
	    glColor3f (0.7, 0.7, 0.7);
	    glVertex3f(4.5f, 0.0f, -1.5f);          // Bottom Right
	    glColor3f (0.7, 0.7, 0.7);
	    glVertex3f(-3.0f, 0.0f, -1.5f);          // Bottom Left
...
void Battle::drawHPMeterMarill(){
    ...
    drawString(215,395,"MARILL           Lv50");

    glColor3f (1.0, 1.0, 1.0);
  	glBegin(GL_QUADS);                  // Draw A Quad
	    glVertex3f(-1.5f, 0.0f, -0.5f);          // Top Left
	    glVertex3f( 4.0f, 0.0f, -0.5f);          // Top Right
	    glVertex3f( 4.0f, 0.0f, -1.0f);          // Bottom Right
	    glVertex3f(-1.5f, 0.0f, -1.0f);          // Bottom Left
  	glEnd();                      // Done Drawing The Quad
...
void Battle::drawHPMeterCharmander(){
	glColor3f(0.1,0.1,0.1);
    drawString(215,395,"CHARMANDER          Lv50");
...
void Battle::drawHPMeterPikachu(){
	glColor3f(0.1,0.1,0.1);
    drawString(775,750,"PIKACHU           Lv50");
    //white background
    glColor3f (1.0, 1.0, 1.0);
  	glBegin(GL_QUADS);                  // Draw A Quad
	    glVertex3f(-2.0f, 0.0f, -0.5f);          // Top Left
...
void Battle::drawHPMeterOddish(){
	glColor3f(0.1,0.1,0.1);
    drawString(775,750,"ODDISH          Lv50");
    //white background
    glColor3f (1.0, 1.0, 1.0);
  	glBegin(GL_QUADS);                  // Draw A Quad
	    glVertex3f(-2.0f, 0.0f, -0.5f);          // Top Left
...
void Battle::drawNotePad(){
    ...
    glPushMatrix();
		...
		glScalef(0.5,0.5,0.5);                                                 //white
	  	glBegin(GL_QUADS);                  // Draw A Quad
		    glVertex3f(0.75f, 0.0f, 0.92f);          // Top Left
		    glVertex3f(2.10f, 0.0f,  0.92f);          // Top Right
		    glVertex3f(2.10f, 0.0f,  0.63f);          // Bottom Right
		    glVertex3f(0.75f, 0.0f, 0.63f);          // Bottom Left
	  	glEnd();                      // Done Drawing The Quad
    glPopMatrix();
}
...
void Battle::main_screen(){
	...
	drawString(50,900,"GARY WANTS TO BATTLE!");
    ...
    if(ashMovement >= 3){                                //draw pikachu
    	...
    	if(ashSentOutPikachuString)
			drawString(50,900,"ASH: PIKACHU I CHOOSE YOU!!!");
		if(!pikachuFainted){
			glPushMatrix();
    		...
    		if(pikachuFaintedString && !bubble && !icebeam && !ember)
    			drawString(50,900,"PIKACHU FAINTED!");
	...
	if(pikachuFainted){
		ashMovement2 += 0.015;
		if(ashMovement2 >= 3){  
			pikachuFaintedString = 0;                            //draw ODDISH
			glColor3f(0.9,0.9,0.9);
			if(ashSentOutOddishString)
				drawString(50,900,"ASH SENT OUT ODDISH!");
			if(!oddishFainted){
	    		glPushMatrix();
			    	glTranslatef(-0.5,2.0,0.3);
			        glRotatef(90,1.0,0.0,0.0);
			       	//glScalef(0.1,0.1,0.1);
			       	//CHARMANDER.enableShade();
					//CHARMANDER.drawCyborg(marillTexture2[0],marillTexture2[1],marillTexture2[2],marillTexture2[3]);
					//CHARMANDER.disableShade();
	    		...
	    		if(oddishFaintedString && !bubble && !icebeam && !ember)
	    			drawString(50,900,"ODDISH FAINTED!");
	    		glPushMatrix();                                        //draw ash again
	    			if(ashMovement3 >= 2.0){
...
//-------------------------------------------------------------------------------------------------------------------	
	...
	if(garyMovement >= 8){                                                  //draw marill
		...
		if(garySentOutMarillString)
			drawString(50,900,"GARY SENT OUT MARILL!");
		if(!marillFainted){
    		glPushMatrix();
    		...
    		if(marillFaintedString && !thunderbolt && !lightball && !razorleaf)
    			drawString(50,900,"MARILL FAINTED!");
	...
	if(marillFainted){
		...
		if(garyMovement2 >= 8){  
			...
			if(garySentOutCharmanderString)
				drawString(50,900,"GARY SENT OUT CHARMANDER!");
			if(!charmanderFainted){
	    		...
	    		if(charmanderFaintedString && !thunderbolt && !lightball && !razorleaf)
	    			drawString(50,900,"CHARMANDER FAINTED!");
	    		glPushMatrix();                                        //draw gary again
				...
				if(!charmanderFaintedString && !thunderbolt)
	    			drawString(50,900,"GARY: WHAT?.....NO! THAT CANT BE ! ITS NOT FAIR !");
	    	}
		}
	}
	if(charmanderFainted)
...
//-------------------------------------------------------------------------------------------------------	
	...
	if(bubble){
		glColor3f(0.9,0.9,0.9);
		//if(marBub)
			drawString(50,900,"MARILL USED BUBBLE!");
    	bubbleMovement += 0.08;
		glPushMatrix();
			if((15.0-bubbleMovement*2.0) <= 1.5){
				glColor3f(0.9,0.9,0.9);
				drawString(50,930,"It's not very effective.");
				//water effect
				glPushMatrix();
					glTranslatef(-0.5,2.0,0.3);
	...
	if(icebeam){
		glColor3f(0.9,0.9,0.9);
		//if(marBub)
			drawString(50,900,"MARILL USED ICEBEAM!");
    	icebeamMovement += 0.08;
		glPushMatrix();
			if((15.0-icebeamMovement*2.0) <= 1.5){
				glColor3f(0.9,0.9,0.9);
				drawString(50,930,"The opponent was frozen.");
				//water effect
				glPushMatrix();
	...
	if(ember){
		glColor3f(0.9,0.9,0.9);
		//if(marBub)
			drawString(50,900,"CHARMANDER USED EMBER!");
    	emberMovement += 0.08;
		glPushMatrix();
			if((15.0-emberMovement*2.0) <= 1.5){
				glColor3f(0.9,0.9,0.9);
				drawString(50,930,"The opponent was burned out.");
				//water effect
				glPushMatrix();
					glTranslatef(0.0,-5.0,0.2);
					glColor3f(0.0,0.0,0.8);
	    	...
	    	/*glTranslatef(5.0-(emberMovement*1.7)/2,15.0-emberMovement*2.0,0.7);
	        gluSphere(h,0.21,10,10);
	        glTranslatef(5.1-(emberMovement*1.7)/2,15.0-emberMovement*2.0,0.7);
	        gluSphere(h,0.2,10,10); 
	        glTranslatef(4.9-(emberMovement*1.7)/2,15.0-emberMovement*2.0,0.7);
	        gluSphere(h,0.1,10,10);*/  
    	glPopMatrix(); 
	}
	//Pikachu's attacks
	if(thunderbolt){
		glColor3f(0.9,0.9,0.9);
		drawString(50,900,"PIKACHU USED THUNDERBOLT!");
    	thunderboltMovement += 0.08;
		glPushMatrix();
			//glColor3f(0.9,0.9,0.0);
			if((2.0+thunderboltMovement*2.0) >= 15.0){
				glColor3f(0.9,0.9,0.9);
				drawString(50,930,"The opponent was paralysed.");
				//electric effect
				glPushMatrix();
	...
	if(lightball){
		glColor3f(0.9,0.9,0.9);
		drawString(50,900,"PIKACHU USED LIGHTBALL!");
	...
	if(razorleaf){
		glColor3f(0.9,0.9,0.9);
		drawString(50,900,"ODDISH USED RAZORLEAF!");
    	razorleafMovement += 0.08;
		glPushMatrix();
			//glColor3f(0.9,0.9,0.0);
			if((2.0+razorleafMovement*2.0) >= 15.0){
				glColor3f(0.9,0.9,0.9);
				drawString(50,930,"It cut through opponent skin.");
				//electric effect
				glPushMatrix();
					glTranslatef(0.0,0.0,2.4);
...
void Battle::mouse(int btn, int state, int x, int y){
	int yy,xx;
	int a0x0=1206,a0x1=1270, a0y0=109,a0y1=134;//exit 1218  y= 76 1280  y= 99
	int bx0=1004,bx1=1090, by0=108,by1=132;//enter 1058  y= 75 1148  y= 99
